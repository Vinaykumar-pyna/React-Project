                ...
                <AppBar position="static">
                    <Toolbar>
                        ...
                        ><MenuItem component={Link} to="/" onClick={handleMenuClose}>
                            ...
                            <MenuItem component={Link} to="/install-app" onClick={handleMenuClose}>
                            ...
                            <AppBar position="static">
                                <div style={{display: "flex"}} className="chat-header">
                                    ...
                                    <Toolbar className={`header-right-icons ${!selectedChat ? 'hide-icons' : ''}`}>
                                        ...
                                        >
                                            <MenuItem component={Link} to="/" onClick={handleMoreVertClose}>
    ...
    return (
        ...
        >
            <Link to={`/chat/${chat.id}`} className="chat-link">
                ...
                <ListItemText
                    primary={<span className="primary-text">{chat.creator.name || "Unknown User"}</span>}
                    secondary={isLoading ? <CircularProgress size={20} /> : <span className="secondary-text">{truncateMessage(lastMessage)}</span>}
                />
            </Link>
        </ListItem>
    );
...
const ChatWindow = ({chatId, handleSendMessage}) => {
            ...
            <div className="messages-container">
                <List className="message-list">
                    {data.data.map((message) => (
                        <ListItem key={message.id} className={message.sender.name ? 'outgoing' : 'incoming'}>
                            <ListItemText
                                primary={message.sender.name || "Unknown User"}
                                secondary={message.message}
                                secondaryTypographyProps={{component: 'span', color: 'textPrimary'}}
                            />
                            <Typography className="timestamp">{formatTimestamp(message.created_at)}</Typography>
                        </ListItem>
                    ))}
                </List>
            </div>
